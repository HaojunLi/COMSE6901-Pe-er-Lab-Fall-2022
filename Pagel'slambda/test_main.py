...
def Seedplant_sanity_assistance():
  ...
  for i in maxH:
      X.append([i])
  X = np.array(X)
  # Modified to test whether x divided by a constant will affect result.
  X = X / 2.5
...
class Test_Ln_Brownian_motion_likelihood(unittest.TestCase):
  ...
  def test_Found_Pagel_Maximumlikelihood(self):
    ...
    ValueSave = Seedplant_sanity_assistance()

    # self.assertAlmostEqual(np.array_equal(ValueSave, np.array([[-206.66,0.49],[58.99, 0.76],[-521.91, 0.91],[-83.07, 0.95],[-18.07, 0.67]])), 1)
    self.assertAlmostEqual(np.array_equal(ValueSave[:,[-1]], np.array([[0.49],[0.76],[0.91],[0.95],[0.67]])), 1)
